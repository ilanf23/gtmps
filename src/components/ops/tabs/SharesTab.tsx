  ...
  return (
    <div className="space-y-4">
      {!data.attribution_wired && (
        <div className="rounded border border-amber-900 bg-amber-950/40 text-amber-300 text-xs px-3 py-2">
          ...
          {data.outbound_legacy.length === 0 ? (
            ...
            <Table
              ...
              rows={data.outbound_legacy.map((s) => [
                <a key="s" href={`/m/${s.slug}`} target="_blank" rel="noreferrer" className="font-mono text-xs text-zinc-300 hover:text-zinc-100 underline-offset-2 hover:underline">/m/{String(s.slug)}</a>,
                String(s.channel),
          ...
          {data.inbound.length === 0 ? (
            ...
            <Table
              ...
              rows={data.inbound.map((v) => [
                <span key="v" className="font-mono text-xs text-zinc-300">/m/{String(v.slug)}</span>,
                <span key="r" className="font-mono text-xs text-zinc-400">/m/{String(v.referrer_slug)}</span>,
        ...
        {data.top_sharers.length === 0 ? (
          ...
          <Table
            ...
            rows={data.top_sharers.map((t) => [
              <a key="s" href={`/m/${t.slug}`} target="_blank" rel="noreferrer" className="font-mono text-xs text-zinc-300 hover:text-zinc-100 underline-offset-2 hover:underline">/m/{t.slug}</a>,
              <span key="c" className="text-zinc-300">{t.downstream_visits}</span>,
            ])}
          />
        )}
      </Section>
    ...
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-zinc-800">
          {rows.map((r, i) => (
            <tr key={i}>{r.map((cell, j) => <td key={j} className="px-2 py-1.5 text-zinc-300">{cell}</td>)}</tr>
          ))}
        </tbody>
      </table>
    </div>
