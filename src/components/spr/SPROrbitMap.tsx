  ...
  return (
    ...
    >
      <svg viewBox="0 0 560 560" className="w-full max-w-[420px] shrink-0">
        <defs>
          <radialGradient id="spr-orbit-glow" cx="50%" cy="50%" r="50%">
        ...
        <circle cx={cx} cy={cy} r={260} fill="url(#spr-orbit-glow)" />
        ...
        <style>{`
          ...
          .spr-ring-pulse { animation: spr-breathe 2s ease-in-out infinite; }
        `}</style>
        ...
        {RINGS.map((ring, i) => (
          <text
            ...
            style={{ transition: "opacity 0.3s", textTransform: "uppercase" as const, filter: `drop-shadow(0 0 4px ${ring.color}66)` }}
          >
            {ring.label}
          </text>
        ))}
        ...
        {RINGS.map((ring, i) => (
          <g key={i}>
            <circle
              ...
              className={ring.pulse ? "spr-ring-pulse" : ""}
              style={{ transition: "stroke-width 0.3s, opacity 0.3s" }}
              filter={ring.pulse && active === i ? "url(#spr-glow)" : undefined}
            />
            <circle
              cx={cx} cy={cy} r={ring.r}
        ...
        <circle cx={cx} cy={cy} r={20} fill="#C65D3E" opacity={0.15} />
        <circle cx={cx} cy={cy} r={14} fill="#1A1A2E" stroke="#C65D3E" strokeWidth={1.5} />
        <text x={cx} y={cy + 4} textAnchor="middle" fill="#C65D3E" fontSize="10" fontWeight="bold" fontFamily="Inter, sans-serif">
          SPR
        </text>
      </svg>
          ...
          >
            ...
            <div>
              <div
                ...
                style={{ color: active === i ? ring.color : "rgba(255,255,255,0.6)", transition: "color 0.3s" }}
              >
                {ring.label}
              </div>
              <div
                className="text-[11px] mt-0.5"
                style={{ color: active === i ? "rgba(255,255,255,0.85)" : "rgba(255,255,255,0.35)", transition: "color 0.3s" }}
              >
                {ring.detail}
              </div>
            </div>
